# 🌌 Galaxy Component Documentation

## Overview
The **Galaxy Scene** is a core immersive component of the AstraGuard Experience, responsible for visualizing the Milky Way galaxy as the top-level navigable environment. It serves as the entry point for the user's journey, allowing them to locate the Solar System and zoom in for granular orbital monitoring.

## 🛠️ Technical Implementation

### Rendering Engine
- **Framework**: `React Three Fiber` (R3F)
- **Geometry**: `Points` system based on a GLTF model (`galaxy.glb`).
- **Performance**: Utilizes `instancedMesh` concepts via `<Points>` for rendering thousands of stars efficiently with a single draw call.

### 🎨 Visual Features

#### 1. Procedural Coloration
Star colors are generated programmatically based on their distance from the galactic center to mimic astrophysical reality:
- **Center**: Redshift/Warm tones (Older stars, higher density).
- **Outer Rim**: Blue/Cool tones (Younger stars, lower density).
- **Logic**:
  ```typescript
  // Distance-based color interpolation
  const normalizedDistance = distanceToCenter / 100;
  color.setRGB(
    Math.cos(normalizedDistance),       // Red channel peaks at center
    MathUtils.randFloat(0, 0.8),        // Green channel variation
    Math.sin(normalizedDistance)        // Blue channel peaks at edges
  );
  ```

#### 2. Solar System Marker
A specific star represents our Solar System, featuring dynamic visual properties:
- **Dynamic Scaling**: The marker scales inversely with camera distance (`1 / distance`) to ensure visibility from the galactic view while shrinking as the user zooms in.
- **Positioning**: Placed at specific coordinates (`GALAXY.SOLAR_SYSTEM_STAR.INIT_POSITION`) relative to the galactic center.

#### 3. Continuous Animation
- The galaxy rotates slowly on the Z-axis (`clock.getElapsedTime() / 15`) to provide a sense of vast scale and motion.

## 🧭 Navigation & Interaction

### Zoom System (`GSAP`)
The component implements a sophisticated camera transition system using **GSAP Timelines**.

1.  **Zoom In Sequence**:
    - **Targeting**: Camera locks onto the Solar System star vector.
    - **Lerping**: Interpolates camera position from `Initial` to `SolarSystem + Offset`.
    - **FOV Adjustment**: Dynamically narrows Field of View ($FOV$) to create a "warp speed" tunnel effect.
    - **Material Transition**: The marker star fades to black (`0x000`) upon close approach, aiming to seamlessly blend into the next scene's background.

2.  **Navigation State**: 
    - Managed via `useSceneStore` and `useNavigation`.
    - Triggers the `endTransition` callback to swap the active scene from **Galaxy** to **Solar System**.

### Mobile Responsiveness
- Adapts camera initialization vectors (`INITIAL_CAMERA_MOBILE_POS` vs `DESKTOP_POS`) to frame the galaxy correctly on portrait and landscape aspect ratios.

## 📦 Dependencies
- `three`: Core 3D library (Vector3, Color, MathUtils).
- `@react-three/fiber`: React reconciler for Three.js.
- `@react-three/drei`: Helpers (`useGLTF`, `Points`).
- `gsap`: Timeline management for complex camera cuts.
div align="center">
